...
router.delete("/:id", auth, async (req, res) => {
    try {
        ...
        let message = post.text.slice(0, 25);
        let ownersName = post.name;
        // Check user for ownership
        if (post.user.toString() !== req.user.id) {
            return res.status(401).json({ msg: "User not authorized" });
        }
        await post.remove();

        res.json({ msg: `Post "${message}" removed! by ${ownersName}` });
    } catch (error) {
        console.error(error.message);
        if (error.kind === "ObjectId") {
            return res.status(404).json({ msg: "Post not found" });
...
router.put("/like/:id", auth, async (req, res) => {
    try {
        ...
        post.dislikes = post.dislikes.filter(
            dl => dl.user.toString() !== req.user.id,
        );
        await post.save();
        res.json(post.likes);
    } catch (error) {
...
router.put("/dislike/:id", auth, async (req, res) => {
    try {
        ...
        post.likes = post.dislikes.filter(
            like => like.user.toString() !== req.user.id,
        );
        await post.save();
...
router.put("/undislike/:id", auth, async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({ msg: "Post has not been disliked" });
        }
        post.dislikes = post.dislikes.filter(
